...
// JSON control-plane payloads ridden inside the 0x40 CHAT_ENVELOPE frame
// (itself ChaCha20-Poly1305 sealed). Wire format identical on Mac + iOS;
// keep this file in sync with GruxPhone/GruxPhone/PhoneChatEnvelope.swift.
//
// Phone → Mac:
//   userMessage    send a user utterance into the active thread
//   switchThread   make threadId the active one on both sides
//   newThread      create a fresh thread (optionally with a title)
//   renameThread   rename
//   deleteThread   remove a thread
//   requestSync    ask Mac to resend the current full state
//
// Mac → Phone:
//   threadList        full sidebar projection
//   threadMessages    the full message array for one thread
//   activeThread      which thread is "current"
//   thinking          Grux is working on a reply
//   assistantMessage  a single new assistant message appended
//   userMessageEcho   a user message appended (useful if another UI adds one)
...
// Tagged-union. Encoded as {"type": "...", "payload": { … }}.
enum ChatEnvelope: Codable {
    ...
    case requestSync
    // Phone "Resume on Mac" tap. The Mac doesn't switch accounts from this
    // alone - it just opens the Resume sheet on the Mac so the user can pick
    // the target account from the desktop. This keeps the OAuth browser flow
    // on the Mac (the only device where claude CLI runs).
    ...
    // Notify the phone that an agent job paused waiting for an account
    // switch. Phone shows a banner + "Resume on Mac" button.
    //   reason: "authLimitHit" today, future-extensible
    ...
    private struct PhaseTransitionedP: Codable {
    ...
    func encode(to encoder: Encoder) throws {
        ...
        case .phaseTransitioned(let cid, let rid, let pn, let pi, let tp, let ts):
            try c.encode("phaseTransitioned", forKey: .type)
            try c.encode(
                PhaseTransitionedP(
                    commandId: cid, runId: rid, phaseName: pn,
                    phaseIndex: pi, totalPhases: tp, ts: ts
                ),
                forKey: .payload
            )
    ...
    init(from decoder: Decoder) throws {
        ...
        case "phaseTransitioned":
            let p = try c.decode(PhaseTransitionedP.self, forKey: .payload)
            self = .phaseTransitioned(
                commandId: p.commandId, runId: p.runId, phaseName: p.phaseName,
                phaseIndex: p.phaseIndex, totalPhases: p.totalPhases, ts: p.ts
            )
